...
import { Button, Tag, Typography,Tabs,Layout,List,Card,Divider,Input, Space } from 'antd';
        ...
        <Content style={{ background:'white' }} > 
                <Tabs defaultActiveKey="1" size="large" type="line" tabBarStyle={{ background:'white' ,paddingLeft:25 }} >
                    ...
                    <ProCard split="horizontal" >
                        ...
                        <Button size="large">开始</Button>--<Button onClick={addNewStage} shape="circle" icon={<PlusOutlined />} size="small"/>
                        ...
                        { allStages.map((item,index)=>{ 
                           return <span key={item.name}>
                                ...
                                --<Button onClick={addNewStage} shape="circle" icon={<PlusOutlined />} size="small"/></span>
                        }) }
                                ...
                                renderItem={item => (
                                    <List.Item>
                                         <Card hoverable bordered 
                                            style={{ textAlign:"center",width:"100%"}} >{item.name}</Card>
                                    </List.Item>
                                )}   footer={<a style={{fontSize:16}}>添加子任务+</a>}  />
                            </ProCard >
                            <ProCard >
                                <span style={{fontSize:16}}>编译命令</span>
                                <Divider />
                                <ProForm  >
                                </ProForm>  
                            </ProCard>
